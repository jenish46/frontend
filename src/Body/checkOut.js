...
import {Component,state,changeHandler,order} from 'react'
      ...
      <ul className="list-group mb-3">
{
    ...
    this.state.data.map((data,i)=>{
        ...
        return(
           ...
           <img src={"http://localhost:3000/images/"+data.food.Image} alt="..." height="50" width="50"className="img-responsive"/>
		
          <div>
         						
            <h6 className="my-0">{(i+1)+" "+data.food.Name}</h6>
            <strong className="text-muted">{"Quantity: "+data.qty}</strong>
          </div>
          <span className="text-muted">${data.food.Price * data.qty}</span>
        </li>
        )
    })
}
...
zoom={15}></Map>
        ...
        <div className="d-block my-3">
          <div className="custom-control custom-radio">
            <input id="credit" name="paymentMethod" type="radio" className="custom-control-input text-left" defaultChecked required />
            <label className="custom-control-label" htmlFor="credit">Credit card</label>
          </div>
          <div className="custom-control custom-radio">
            <input id="debit" name="paymentMethod" type="radio" className="custom-control-input text-left" required />
            <label className="custom-control-label" htmlFor="debit">Debit card</label>
          </div>
          <div className="custom-control custom-radio">
            <input id="paypal" name="paymentMethod" type="radio" className="custom-control-input text-left" required />
        ...
        <hr className="mb-4" />
        <button className="btn btn-primary btn-lg btn-block" type="submit" onClick={this.order}>Continue to checkout</button>
      </form>
    </div>
  </div>
