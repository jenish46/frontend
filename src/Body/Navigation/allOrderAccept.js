...
export default class allOrder extends Component {
  ...
  componentDidMount() {
    ...
    if(type==="Admin")
    {
      axios.get('http://localhost:3000/unApproved', this.state.config).then((response) => {
        this.setState({
  ...
  axios.get('http://localhost:3000/unDelivered', this.state.config).then((response) => {
    this.setState({
      data: response.data.data
    })
})
  ...
  acceptOrder = (id) => {
    axios.put('http://localhost:3000/approve/' + id, this.state, this.state.config).then((response) => {
toast.success("Order Accepted")
  ...
  deliverOrder=(oid,lat,lng)=>{
    axios.put('http://localhost:3000/Acceptdelivery/'+oid,this.state,this.state.config).then((response)=>{
                          ...
                          <div className="row">
                            <div className="col-12">
                              <div className="text-center text-150">
                                ...
                                <span className="text-default-d3">FoodHunter</span>
                              </div>
                            </div>
                          </div>
                          {/* .row */}
                          ...
                          <div className="mt-4">
                            ...
                            <div className="text-95 text-secondary-d3">
...

                              {
                                ...
                                data.ProductId.map((food, i) => {
                                  return (
                                    <>
                                      <div className="row mb-2 mb-sm-0 py-25">
                                        <div className="d-none d-sm-block col-1">{i}</div>
                                        <div className="col-9 col-sm-5">{food.food.Name}</div>
                                        <div className="d-none d-sm-block col-2">{food.qty}</div>
                                        <div className="d-none d-sm-block col-2 text-95">${food.food.Price}</div>
                                        <div className="col-2 text-secondary-d2">{food.food.Price * food.qty}</div>
                                      </div>
